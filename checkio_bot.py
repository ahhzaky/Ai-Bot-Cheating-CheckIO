...
class CheckIOSolver:
    ...
    def solve_tasks_on_island(self, tasks_to_solve):
        for task in tasks_to_solve:
            if self.solve_one_task(task):
                print(task.name, ": cant't be solved :(")
            else:
    ...
    def solve_and_check_task(self, solution_code, task_obj):
        ...
        try:
            ...
            self.driver.find_element_by_xpath("//a[@class='btn']").click()
        time.sleep(2)
        solution_textarea_element = self.driver.find_element_by_xpath(
            "//textarea")
        ...
        for current_attempt in range(20):
            success_element = self.driver.find_elements_by_xpath(
    ...
    def get_google_solutions_for_task(self, task):
        ...
        google_results = self.driver.find_elements_by_xpath(
            "//div[@class='rc']//a[contains(@href,'publications')]")
    ...
    def get_unsolved_tasks_from_island(self, link_to_island):
        ...
        for task in tasks:
            try:
                task_status = task.find(
                    class_='island-tasks__side__sign').get('title')
                if task_status != 'Solved' and task_status != 'Blocked':
                    title = task.find(
                        class_='island-tasks__task__title').get('title')
...
if __name__ == '__main__':
    ...
    # bot.get_google_solutions_for_task(Task('Easy Unpack', '...'))
